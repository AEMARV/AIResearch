...
class Experiment:
	...
	def train_all(self,model,
		...
		for epoch in range(optimizer.trained_epochs+1,epochnum):
			if not ISNAN:
				epochres,ISNAN = self.train_epoch(model,optimizer,trainldr,testldr)
			optimizer.inc_epoch()
...
class Trainings_SimpleCIFAR10(Experiment):
	...
	def generic_train(self,trial=1, alpha=2,
					  ...
					  init_coef=1,
					  model=None,
					  optimizer=Joint_Probabilistic,
					  dataset=cifar10,name="",
					  classnum=-1):
		self.reset_path()
		model = model(layers=num_layer,classnum=classnum,filter_scale=filter_scale,init_coef=init_coef)
		optimizer = optimizer(alpha, classnum, augment_rate, lr=lr,momentum=momentum,l1=l1,l2=l2)
